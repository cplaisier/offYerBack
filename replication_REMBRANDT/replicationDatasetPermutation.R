...
ratSec <- read.delim( file=gzfile('mergedREMBRANDT_groups.csv.gz'), sep=",", as.is=T, header=T,row.names=1 )
...
ratSec.all <- ratSec[,c(rownames(rug2)[c(which(rug2[,1]=='GBM'),which(rug2[,1]=='OLIGODENDROGLIOMA'),which(rug2[,1]=='MIXED'),which(rug2[,1]=='ASTROCYTOMA'))],controls2)]
...
permutations = 1000
p1 = read.csv('phenotypes_REMBRANDT.csv',header=T,row.names=1)
for(k in 1:ks) {
    ...
    if(length(k.rows.sec)>1) {
        ...
        scph1 = summary(coxph(Surv(SURVIVAL,DEAD) ~ pc.1.all, data=d2))
        m1[k,19] = scph1$coef[1,4]
        m1[k,20] = scph1$coef[1,5]
        scph2 = summary(coxph(Surv(SURVIVAL,DEAD) ~ pc.1.all + AGE, data=d2))
