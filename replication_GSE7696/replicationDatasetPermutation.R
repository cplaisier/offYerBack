...
ratSec <- read.delim( file=gzfile('mergedGSE7696_groups.csv.gz'), sep=",", as.is=T, header=T,row.names=1 )
...
ratSec.gbm <- ratSec[,c(rownames(rug2)[c(which(rug2[,1]=='GBM'))],controls2)]
...
outNames = c('n.rows','overlap.rows','new.resid.norm.gbm','avg.norm.perm.resid.gbm','norm.perm.p.gbm','pc1.var.exp.gbm','avg.pc1.var.exp.gbm','pc1.perm.p.gbm','survival.gbm','survival.p.gbm','survival.age.gbm','survival.age.p.gbm')
m1 = matrix(ncol=length(outNames),nrow=ks,dimnames=list(1:ks,outNames))
permutations = 1000
p1 = read.csv('phenotypes_GSE7696.csv',header=T,row.names=1)
for(k in 1:ks) {
    ...
    if(length(k.rows.sec)>1) {
        ...
        m1[k,6] = ((tmp.pc.1.gbm$sdev^2)/sum(tmp.pc.1.gbm$sdev^2))[1]
        ...
        scph1 = summary(coxph(Surv(SURVIVAL,DEAD) ~ pc.1.gbm, data=d2))
        m1[k,9] = scph1$coef[1,4]
        m1[k,10] = scph1$coef[1,5]
        scph2 = summary(coxph(Surv(SURVIVAL,DEAD) ~ pc.1.gbm + AGE, data=d2))
        m1[k,11] = scph2$coef[1,4]
        m1[k,12] = scph2$coef[1,5]
    }
}
