...
class pssm:
    ...
    def getMemeFormatted(self,atFreq=0.25,cgFreq=0.25):
        ...
        if self.de_novo_method=='meme':
            ...
            eValue = 0.05
        memeFormatted += 'BL   MOTIF '+self.name+' width=0 seqs='+str(int(nsites)+1)+'\n'
        memeFormatted += 'letter-probability matrix: alength= 4 w= '+str(len(self.matrix))+' nsites= '+str(int(nsites)+1)+' E= '+str(eValue)
